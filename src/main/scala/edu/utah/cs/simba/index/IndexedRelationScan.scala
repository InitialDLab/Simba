    ...
    val after_filter = if (predicates.size == 1 && predicates.head.toString == "true"){
      ...
      case treemap @ TreeMapIndexedRelation(_, _, _, column_keys, _) =>
        if (predicates.nonEmpty) {
          ...
          val broadcastInterval = simbaContext.sparkContext.broadcast(intervals)
          // local index
        ...
        if (predicates.nonEmpty) {
          val res_rdds = predicates.map { predicate =>
            ...
            var global_part = rtree.global_rtree.range(queryMBR).map(_._2).toSeq
              ...
              if (knn_res != null) simbaContext.sparkContext.parallelize(knn_res, 1).intersection(tmp_rdd)
              else tmp_rdd
            } else {
              ...
              simbaContext.sparkContext.parallelize(final_res, 1)
            }
          }
