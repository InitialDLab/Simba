  ...
  def applyCondition(rdd: RDD[InternalRow], condition: Expression): RDD[InternalRow] = {
    condition match {
      ...
      case now@And(left, right) =>
        if (!now.hasKNN) rdd.mapPartitions{ iter => iter.filter(newPredicate(condition, child.output))}
      ...
      case _ =>
        rdd.mapPartitions(iter => iter.filter(newPredicate(condition, child.output)))
    }
  }
