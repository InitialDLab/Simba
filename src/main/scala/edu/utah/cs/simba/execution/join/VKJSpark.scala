...
import edu.utah.cs.simba.partitioner.{MapDPartition, VoronoiPartition}
import edu.utah.cs.simba.spatial.Point
import edu.utah.cs.simba.util.{BoundedPriorityQueue, ShapeUtils}
...
case class VKJSpark(left_key: Expression, right_key: Expression, l: Literal,
  ...
  final val num_of_pivot_sets = simbaContext.simbaConf.voronoiPivotSetSize
  final val num_of_pivots = num_partitions * simbaContext.simbaConf.thetaBoost
