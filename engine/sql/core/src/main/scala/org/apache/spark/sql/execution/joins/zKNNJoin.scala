...
import org.apache.spark.sql.partitioner.{RangePartition, RangeDPartition, MapDPartition}
...
case class zKNNJoin(
  ...
  def num_partition = sqlContext.conf.numShufflePartitions
  def num_shifts = sqlContext.conf.zknnShiftTimes
  def dimension = left_keys.length
  def shift_vec = genRandomShiftVectors(dimension, num_shifts)
  ...
  def zKNNPerIter(left_rdd: RDD[(Point, InternalRow)], right_rdd: RDD[(Point, InternalRow)], k: Int, shift: Array[Int])
  : RDD[(InternalRow, Array[(InternalRow, Double)])] = {
    ...
    left_partitioned_rdd.zipPartitions(right_dup_partitioned) {
      (leftIter, rightIter) =>  {
