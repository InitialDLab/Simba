...
private[spark] object SQLConf {
  ...
  //Join parameters
  val DISTANCE_JOIN_METHOD = "spark.sql.joins.distanceJoinMethod"
  val KNN_JOIN_METHOD = "spark.sql.joins.knnJoinMethod"

  //RTree parameters
  val MAX_ENTRIES_PER_NODE = "spark.sql.spatial.maxEntriesPerNode"

  //ZKNN Join parameters
  //val JOIN_PARTITIONS = "spark.sql.joins.partitions"
  val ZKNN_SHIFT_TIMES = "spark.sql.joins.shiftTimes"

  val VORONOI_PIVOTSET_SIZE = "spaek.sql.joins.voronoi.pivotSetSize"

  // threshold determine where rtree index using local index or brute force filter
  val INDEX_SIZE_THRESHOLD = "spark.sql.index.threshold"

  val SAMPLE_RATE = "spark.sql.sampleRate"

  val THETA_BOOST = "spark.sql.joins.thetaBoost"
...
private[sql] class SQLConf extends Serializable {
  ...
  //Join parameters
  private[spark] def distanceJoinMethod : String =
    getConf(DISTANCE_JOIN_METHOD, "RTreeDistanceJoin")
  private[spark] def knnJoinMethod : String =
    getConf(KNN_JOIN_METHOD, "RTreeKNNJoin")

  private[spark] def voronoiPivotsetSize: Int = getConf(VORONOI_PIVOTSET_SIZE, "10").toInt

  //RTree parameters
  private[spark] def maxEntriesPerNode : Int =
    getConf(MAX_ENTRIES_PER_NODE, "25").toInt
  private[spark] def indexSizeThreshold : Int =
    getConf(INDEX_SIZE_THRESHOLD, "1000").toInt

  private[spark] def sampleRate: Double = {
    getConf(SAMPLE_RATE, "0.03").toDouble
  }

  private[spark] def thetaBoost: Int = {
    getConf(THETA_BOOST, "16").toInt
  }

  private[spark] def zknnShifts : Int =
    getConf(ZKNN_SHIFT_TIMES, "2").toInt
