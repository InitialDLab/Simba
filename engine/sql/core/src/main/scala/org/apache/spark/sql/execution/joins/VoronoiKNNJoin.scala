...
case class VoronoiKNNJoin(left_keys: Seq[Expression],
  ...
  def generatePivots(origin: RDD[Point], num_pivots : Int) = {
    val sampled = origin.takeSample(withReplacement = false, num_of_pivot_sets * num_pivots, System.currentTimeMillis())
  ...
  def knnObjectForSinglePoint(left_item: (Point, InternalRow),
                              left_pid: Int,
                              sort_by_dist: IndexedSeq[(Int, Double)],
                              pivots: Array[Point],
                              right_meta_info: Array[RightMetaInfo],
                              right_data_by_pivot: Array[Array[(Point, InternalRow, Double)]]): Iterable[InternalRow]= {
    ...
    right_meta_info.foreach(right => {
      if (right_data_by_pivot(right.partitionId) != null){
        right.knnOfPivot.foreach {d =>
          val tempDist = dist_to_pivot(right.partitionId) + d
          theta_pq += tempDist
        }
      }
    })
    ...
    var knn_of_left_item = new BoundedPriorityQueue[(InternalRow, Double)](k)(new DescendOrderRowDistance)
        ...
        if (leftObjectToHP <= theta) {
          ...
          loop.breakable{
            for (i <- right_data_by_pivot(cur_part._1).indices) {
              ...
              if (dist_to_pivot(cur_part._1) > rightItem._3 + theta)
                loop.break()
  ...
  def knnObjectForEachPartition(left_data_by_pivot: Array[Array[(Point, InternalRow)]],
                                right_data_by_pivot: Array[Array[(Point, InternalRow, Double)]],
                                pivots: Array[Point],
                                right_meta_info: Array[RightMetaInfo]): mutable.ListBuffer[InternalRow] ={
    ...
    for (left_pid <- left_data_by_pivot.indices) { // foreach left partition
      if (left_data_by_pivot(left_pid) != null) {
        ...
        leftData.foreach(leftItem =>
          res ++= knnObjectForSinglePoint(leftItem, left_pid, sortByDist, pivots, right_meta_info, right_data_by_pivot)
        )
      }
    }
    res
  ...
  override def doExecute() = {
    ...
    tmp_left_meta_info.foreach(left => {
      ...
      left_meta_info(left.partitionId) = left
    })
    ...
    for (i <- grouping.indices) {
      ...
      right_meta_info.foreach(right => {
        ...
        lower_bounds(i)(right.partitionId) = minLB
      })
    }
    ...
    left_partitioned.zipPartitions(right_dup_partitioned) {
      (leftIter, rightIter) => {
        ...
        if (rightIter.hasNext) {
          ...
          ans ++= knnObjectForEachPartition(leftDataByPivot, rightDataByPivot, bc_pivots.value, right_meta_info)
        }
        ans.iterator
      }
    }
