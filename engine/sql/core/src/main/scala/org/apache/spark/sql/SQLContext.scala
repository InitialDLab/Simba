...
class SQLContext(@transient val sparkContext: SparkContext)
  ...
  def indexTable(tableName: String, indexType: IndexType, indexName: String, column: List[Attribute]): Unit = indexManager.createIndex(tableName, indexType, indexName, column)
  ...
  def dropIndexTableByName(tableName: String, indexName: String): Unit = indexManager.dropIndexByName(tableName, indexName)
  ...
  private[sql] def registerDataFrameAsTable(df: DataFrame, tableName: String): Unit = {
    if (catalog.tableExists(Seq(tableName)))
      System.err.println("Table name has already been registered.")
    else catalog.registerTable(Seq(tableName), df.logicalPlan)
  }
  ...
  def dropTempTable(tableName: String): Unit = {
    ...
    indexManager.tryDropIndexQuery(table(tableName))
