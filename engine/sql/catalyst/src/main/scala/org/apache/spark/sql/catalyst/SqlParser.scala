...
class SqlParser extends AbstractSparkSQLParser {
  ...
  protected lazy val joinConditions: Parser[Expression] =
    ( ON ~> (POINT ~> "(" ~> repsep(termExpression, ",")  <~ ")") ~ (IN ~ KNN ~ "(" ~ POINT ~ "(" ~> repsep(termExpression, ",") <~ ")") ~ ("," ~> literal <~ ")") ^^
  ...
  protected lazy val comparisonExpression: Parser[Expression] =
    ( (POINT ~> "(" ~> repsep(termExpression, ",")  <~ ")") ~ (IN ~ RANGE ~ "(" ~ POINT ~ "(" ~> repsep(termExpression, ",") <~ ")" ~ ",") ~ (POINT ~> "(" ~> repsep(termExpression, ",") <~ ")") <~ ")" ^^
      { case point1 ~ boundL ~ boundR => InRange(point1, boundL, boundR) }
    | (POINT ~> "(" ~> repsep(termExpression, ",")  <~ ")") ~ (IN ~ KNN ~ "(" ~ POINT ~ "(" ~> repsep(literal, ",") <~ ")") ~ ("," ~> literal <~ ")") ^^
      {case point ~ target ~ l => InKNN(point, target, l)}
    | (POINT ~> "(" ~> repsep(termExpression, ",")  <~ ")") ~ (IN ~ CIRCLERANGE ~ "(" ~ POINT ~ "(" ~> repsep(termExpression, ",") <~ ")") ~ ("," ~> literal <~ ")") ^^
