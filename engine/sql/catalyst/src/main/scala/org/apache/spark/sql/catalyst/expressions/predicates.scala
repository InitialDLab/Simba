...
trait PredicateHelper {
  private def _toDNF(condition: Expression): Expression = {
    condition match {
      case Or(left, right) => {
        val newLeft = _toDNF(left)
        val newRight = _toDNF(right)
        Or(newLeft, newRight)
      }
      case And(left, right) => {
        ...
        newLeft match {
          case Or(l, r) =>
            ans = Or(And(l, newRight), And(r, newRight))
          case _ =>
        }
        newRight match{
          case Or(l, r) =>
            if (ans == null)
              ans = Or(And(newLeft, l), And(newLeft, r))
          case _ =>
        }
        if (ans == null)
  ...
  private def _toCNF(condition: Expression): Expression = {
    condition match {
      case And(left, right) => {
        val newLeft = _toCNF(left)
        val newRight = _toCNF(right)
        And(newLeft, newRight)
      }
      case Or(left, right) => {
        var ans: Expression = null
        ...
        newLeft match {
          case And(l, r) =>
            ans = And(Or(l, newRight), Or(r, newRight))
          case _ =>
        }
        newRight match{
          case And(l, r) =>
            if (ans == null)
              ans = And(Or(newLeft, l), Or(newLeft, r))
          case _ =>
        }
        if (ans == null)
          Or(newLeft, newRight)
  ...
  protected def toCNF(condition: Expression): Expression = {
    //TODO _toCNF do not process duplication
//    val hasReplication = _toCNF(condition)
//    cnfExtract(hasReplication).map(splitDisjunctivePredicates(_).distinct
//      .reduceLeftOption(Or).getOrElse(Literal(true))).reduceLeftOption(And).getOrElse(Literal(true))
    _toCNF(condition)
  }

  protected def toDNF(condition: Expression): Expression = {
//    val hasReplication = _toDNF(condition)
//    dnfExtract(hasReplication).map(splitConjunctivePredicates(_).distinct
//      .reduceLeftOption(And).getOrElse(Literal(true))).reduceLeftOption(Or).getOrElse(Literal(true))
    _toDNF(condition)
  }
  ...
  protected def cnfExtract(expression: Expression) : Seq[Expression] = {
    expression match {
      ...
      case Or(left @ Or(l2, r2), right) =>
        cnfExtract(Or(l2, Or(l2, right)))
      case other =>
        other :: Nil
    }
  }

  protected def splitCNFPredicates(condition: Expression): Seq[Expression] = {
    val newCondition = _toCNF(condition)
    cnfExtract(newCondition)
  }

  protected def splitDNFPredicates(condition: Expression): Seq[Expression] = {
    val newCondition = _toDNF(condition)
    dnfExtract(newCondition)
  }
...
case class InRange(point: Seq[Expression], pointL: Seq[Expression], pointR: Seq[Expression]) extends  Predicate {
  override def hasSpatial = true
  override def children:  Seq[Expression] = point ++ pointL ++ pointR

  override def nullable: Boolean = true // TODO: Figure out correct nullability semantics of IN.
  override def toString: String = s" **($point) IN Rectangle ($pointL) - ($pointR)**  "
...
case class InKNN(point: Seq[Expression], target: Seq[Expression], k: Literal) extends Predicate{
  ...
  override def toString: String = s" **$point is INKNN($target, $k)** "
...
case class InCircleRange(point: Seq[Expression], target: Seq[Expression], r: Literal) extends Predicate{
  ...
  override def nullable = true
  override def toString: String = s" **$point is InCircleRange2($target, $r)** "
