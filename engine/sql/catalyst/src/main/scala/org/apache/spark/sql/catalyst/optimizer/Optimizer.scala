...
object PredicateToCNF extends Rule[LogicalPlan] with PredicateHelper{
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    case Filter(condition: Expression, child: LogicalPlan) =>
      Filter(toCNF(condition), child)
  }
}
...
object PushPredicateThroughIndex extends Rule[LogicalPlan] with PredicateHelper{
  def apply(plan: LogicalPlan): LogicalPlan = plan transform {
    // TODO Select operations can be processed by indexes
    // NOTE: IndexedRelation should be generated before this Rule
    // Why I can not use org.apache.spark.sql.index ??

//    case f @ Filter(filterCondition, IndexedRelation())
    null
  }
}
