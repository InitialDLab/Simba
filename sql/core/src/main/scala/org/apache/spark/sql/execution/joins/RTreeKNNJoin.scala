...
/**
 * Created by Dong Xie on 10/1/15.
 *
 */
case class RTreeKNNJoin(
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    val (left_partitioned, left_mbr_bound) =
      STRPartition(left_rdd, dimension, num_partitions, sample_rate, transfer_threshold, max_entries_per_node)

    val right_sampled = right_rdd.sample(withReplacement = false, sample_rate, System.currentTimeMillis()).map(_._1)
      ...
      (leftIter, rightIter) =>
        ...
        if (right_data.length > 0) {
          ...
          leftIter.foreach(now =>
            ans ++= right_index.kNN(now._1, k, keepSame = false).map(x => new JoinedRow(now._2, right_data(x._2)._2))
          )
        }
        ans.iterator
    }
