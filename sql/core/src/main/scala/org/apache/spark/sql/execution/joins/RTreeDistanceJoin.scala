...
case class RTreeDistanceJoin(
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    val left_part_size = left_partitioned.mapPartitions {iter => Array(iter.length).iterator}.collect()

    val left_rt = RTree(left_mbr_bound.zip(left_part_size).map(x => (x._1._1, x._1._2, x._2)), max_entries_per_node)
