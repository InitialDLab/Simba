 ...
 * Index Manager for Spark SQL
 */
private case class IndexedData(name: String, plan: LogicalPlan, indexedData: IndexedRelation)

case class IndexInfo(tableName: String, indexName: String, attributes: Seq[Attribute],
                     indexType: IndexType, location: String, storageLevel: StorageLevel) extends  Serializable
...
private[sql] class IndexManager(sqlContext: SQLContext) extends Logging {
  ...
  def hasIndex(tableName: String, indexName : String): Boolean =
    lookupIndexedData(sqlContext.table(tableName), indexName).nonEmpty

  def showIndex(tableName: String): Unit = showQuery(tableName)

  def createIndex(tableName : String, indexType : IndexType, indexName: String, column: List[Attribute]) : Unit =
    createIndexQuery(sqlContext.table(tableName), indexType, indexName, column, Some(tableName))

  def dropIndexByName(tableName: String, indexName : String): Unit =
    dropIndexByNameQuery(sqlContext.table(tableName), indexName)
  ...
  private[sql] def persistIndex(indexName: String, fileName: String): Unit = {
    ...
    sqlContext.sparkContext.parallelize(Array(indexedItem.plan)).saveAsObjectFile(fileName + "/plan")
    ...
    if (preData.indexType == RTreeType) {
      ...
      sqlContext.sparkContext.parallelize(Array(rtreeRelation)).saveAsObjectFile(fileName + "/rtreeRelation")
      ...
      sqlContext.sparkContext.parallelize(Array(treeMapRelation)).saveAsObjectFile(fileName + "/treeMapRelation")
  ...
  private[sql] def loadIndex(indexName: String, fileName: String): Unit = {
    ...
    if (info.indexType == RTreeType){
      val rtreeRelation = sqlContext.sparkContext.objectFile[RTreeIndexedRelation](fileName + "/rtreeRelation")
        .collect().head
      ...
      val treeMapRelation = sqlContext.sparkContext.objectFile[TreeMapIndexedRelation](fileName + "/treeMapRelation")
        .collect().head
      indexedData += IndexedData(indexName, plan,
  ...
  private[sql] def setStorageLevel(query: DataFrame,
    ...
    val dataIndex = indexedData
      .indexWhere(cd => query.queryExecution.analyzed.sameResult(cd.plan) && cd.name.equals(indexName))
    require(dataIndex >= 0, "Index not found!")
    val preData = indexInfos(dataIndex)
    indexInfos(dataIndex) =
      IndexInfo(preData.tableName, preData.indexName, preData.attributes, preData.indexType, preData.location, newLevel)
  }
  ...
  private[sql] def dropIndexByNameQuery(query: DataFrame, indexName: String, blocking: Boolean = true)
  : Unit = writeLock {
    val planToIndex = query.queryExecution.analyzed
  ...
  private[sql] def dropIndexByColumnQuery(query: DataFrame, column: List[Attribute], blocking: Boolean = true)
  : Unit = writeLock {
    val planToIndex = query.queryExecution.analyzed
    var dataIndex = -1
